...
if($q != '') {
    ...
    // picked up and echoed in the search box by the head.php file.
    ...
    $matches = $pages->find("parent=/news/, sort=-date, template=post, title|tags.title|category.title|body%=$q");
                      ...
                      <?php
                        ...
                        if(!$data = $cache->get("blog_topics", 29990)) {
...

                            {
                                ...
                                $blog_count = $pages->count("parent=/news/,  category=$category, template=post");
