...
function renderPosts($posts, $small = false, $home = false) {
...

    {
        ...
        $t = new TemplateFile(wire('config')->paths->templates . '/markup/lifegroups/posts.php');
...
function renderComments(CommentArray $comments, $limit = 0) {
    ...
    if($limit && (!empty($comments) > $limit || $pageNum > 1)) {
        ...
        if(!empty($comments) > $limit) $nextURL = $page->url . 'page' . ($pageNum+1);
